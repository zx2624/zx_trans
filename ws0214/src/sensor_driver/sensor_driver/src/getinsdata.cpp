...
//		ins_data_.dHeading=360-ins_data_.dHeading;
		...
		ins_data_.dAccx=m_INSFons.m_sFONSData.dAccx;
		ins_data_.dAccy=m_INSFons.m_sFONSData.dAccy;
		ins_data_.dAccz=m_INSFons.m_sFONSData.dAccz;
		ins_data_.dArx=m_INSFons.m_sFONSData.dArx;
		ins_data_.dAry=m_INSFons.m_sFONSData.dAry;
		ins_data_.dArz=m_INSFons.m_sFONSData.dArz;
...
//    subConfig = node_handle_.subscribe<std_msgs::String>("startconfig",2, boost::bind(&Node::subStartConfigHandle,this,_1));
  ...
  while(status)
  {
      ...
      if(framenum==0)
	{
      ...
      gps.gps.header.stamp = ros::Time::now();
