...
{
	  auto file_resolver = common::make_unique<
	      common::ConfigurationFileResolver>(
...
public:
  ...
  void cloud_callback (const CloudConstPtr& cloud,int id)
  {
	  ...
	  if(id%2==0)
	  {
		...
		cloudwithid->processcloud = *cloud;
	  if(id%2==1||replay_==2)
		  cloudwithid->update = true;
	  cloudwithid_.Push(cloudwithid);
  }
  ...
  {
	int cloudnum = 0;
	for(auto it=finalcloudmap_.begin();it!=finalcloudmap_.end();it++)
	{
		if(it->second->update)
			cloudnum++;
	}
...
//			LOG(INFO)<<"count:"<<count;
			for(int i=0;i<count;i++)
			{
				...
				tempcloudwithid.stamp = playback_.NowTime(stamp);
				tempcloudwithid.cloud.swap(laserCloud);
				tempcloudwithid.cloud.header.stamp = tempcloudwithid.stamp * 1000000;

				tempcloudwithid.id = id;

				tempcloudmap[id]= std::make_shared<CloudwithID>(tempcloudwithid);

			}
			playback_.EndLoadLine();

			for(auto it=tempcloudmap.begin();it!=tempcloudmap.end();it++)
			{
				int id = it->first;
				...
				cloud_callback(CloudConstPtr(new Cloud(it->second->cloud)),id);
			}
		}
	}
...
//			LOG(INFO)<<"got ins data";
			...
			imuout.header.stamp = ros::Time::now();
			imuout.header.frame_id="imu_frame";
			imuout.linear_acceleration.x = m_AnalysisINS_OxT.INSData_struct.dAccz;
			imuout.linear_acceleration.y = m_AnalysisINS_OxT.INSData_struct.dAccx;
			imuout.linear_acceleration.z = m_AnalysisINS_OxT.INSData_struct.dAccy;

			imuout.angular_velocity.x = m_AnalysisINS_OxT.INSData_struct.dGyroX*M_PI/180;
			imuout.angular_velocity.y = m_AnalysisINS_OxT.INSData_struct.dGyroY*M_PI/180;
...
//			LOG(INFO)<<"publish ins data";
  ...
  bool init()
  {
	...
	if(calibrationmode_)
		calibrationpcddir_ = common::createstampeddir("~/calibrationlidar") + "/";
	...
	//		  hdlCalibration_ =pkgPath+ "/config/S2_64db.xml";//
			intriparacalibration =pkgPath+ "/config/64S3db.xml";//塔河比赛丰田
...
//	  convertcloudmsg();
  ...
  void mixcloud()
  {
	...
	for(auto it=finalcloudmap_.begin();it!=finalcloudmap_.end();it++)
	{
		...
		if(i==0)
		{
			totalcloud_ .swap(it->second->processcloud);
			totalorigincloud_.swap(it->second->cloud);
			maxstamp = it->second->processcloud.header.stamp;
		}
		else
		{
			totalcloud_ += it->second->processcloud;
			totalorigincloud_ += it->second->cloud;
	...
	for(auto it=finalcloudmap_.begin();it!=finalcloudmap_.end();it++)
	{
		...
		totalorigincloud_.push_back(temppoint);
	}
	totalcloud_.header.stamp = maxstamp;
	totalorigincloud_.header.stamp = maxstamp;
	finalcloudmap_.clear();
  }
...
//      }
 ...
 void preprocess()
 {
	 ...
	 LOG(INFO)<<"start";
	 std::vector<bool> obtainflag(lidarnum_,false);
	 while((cloudwithid = cloudwithid_.PopWithTimeout(common::FromSeconds(interval)))!=nullptr)
   {
//		if(playback_.RecordIsOn()&&cloudwithid->id%2==0)
//			Record(cloudwithid);
	   int lidarid = cloudwithid->id/2;
	   LOG(INFO)<<"lidarid:"<<lidarid;
	   if(replay_!=2&&cloudwithid->id%2==0)
	   {
		   addcloudinfo(cloudwithid->cloud,
				   grabberunitmaps_[lidarid].grabber->indexmaptable,grabberunitmaps_[lidarid].lasernum);
		   finalcloudmap_[lidarid] = cloudwithid;
		   LOG(INFO)<<"lidarid:"<<lidarid;
		   continue;
	   }
	   updated_=true;
	   obtainflag[lidarid] = true;
	   if(replay_==2)
	   {
		   cloudwithid->processcloud=cloudwithid->cloud;
		   addcloudinfo(cloudwithid->cloud,
		   				   grabberunitmaps_[lidarid].grabber->indexmaptable,grabberunitmaps_[lidarid].lasernum);

		   precomputecloud(cloudwithid->processcloud,grabberunitmaps_[lidarid].calibvalue);

		   pcl::transformPointCloud (cloudwithid->processcloud, cloudwithid->processcloud,
				   	   	   	   	   grabberunitmaps_[lidarid].transform_matrix_Lidar2Vehicle_);
		   filtercloudonvehicle(cloudwithid->processcloud,grabberunitmaps_[lidarid].calibvalue);
		   addcloudinfo(cloudwithid->processcloud,
				   grabberunitmaps_[lidarid].grabber->indexmaptable,grabberunitmaps_[lidarid].lasernum);
		   finalcloudmap_[lidarid] = cloudwithid;
	   }
	   else
	   {
		   ...
		   if(finalcloudmap_.find(lidarid)==finalcloudmap_.end())
			   finalcloudmap_[lidarid] = cloudwithid;
		   else
		   {
			   finalcloudmap_[lidarid]->processcloud.swap(cloudwithid->processcloud);
			   LOG(INFO)<<"lidarid:"<<lidarid;
			   finalcloudmap_[lidarid]->processcloud.header = cloudwithid->processcloud.header;
		   }

	   }
	   LOG(INFO)<<"lidarid:"<<lidarid;
	   finalcloudmap_[lidarid]->update = true;
	   if((lidarid == lidarnum_-1)
			   &&std::find(obtainflag.begin(),obtainflag.end(),false)==obtainflag.end())
		   break;
   }
...
private:
  ...
  common::BlockingQueue<std::shared_ptr<CloudwithID>> cloudwithid_;
  common::BlockingQueue<std::shared_ptr<std::pair<int,char*>>> insdatas_;
  common::BlockingQueue<std::shared_ptr<std::pair<int,char*>>> ecudatas_;

  std::map<int,std::shared_ptr<CloudwithID>> finalcloudmap_;
...
{
  ros::init(argc, argv, "getmutivelodynedata");
  ros::NodeHandle nh;
