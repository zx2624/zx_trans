...
//	                    cloud_viewer_->spin ();
	...
	void PostProcess::laserOdometryHandler(const nav_msgs::Odometry::ConstPtr& laserOdometry)  //雷达里程计
	{
	  double timeOdometry = laserOdometry->header.stamp.toSec();
	...
	//  double roll, pitch, yaw;
	  ...
	  Eigen::Vector3d translation(laserOdometry->pose.pose.position.x,
...
//		LOG(INFO)<<"starttime"<<ros::Time::now().toSec() - timeLaserCloudFullRes;
	...
	void PostProcess::analysisCloud(pcl::PointCloud<PointType>::Ptr inputcloud,
			std::vector<pcl::PointCloud<PointType>::Ptr>& outputclouds,std::vector<PointType>& lidarpropertys)
	{
		  ...
		  {
			int length;
			pcl::PointCloud<PointType>::Ptr lasercloudptr(new pcl::PointCloud<PointType>);//每一包点云
	...
	void PostProcess::countogmpoints(pcl::PointCloud<PointType>::Ptr cloud)
	{
//		std::cout<<".................................?????????????????????";		std::cout<<".................................?????????????????????";
...
//		std::cout<<"iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"<<endl;
...
//		for(int i=20/point_count_ogm_.ogmresolution+3;i<45/point_count_ogm_.ogmresolution;i++)//20/point_count_ogm_.ogmresolution+3
//		{
////			std::cout<<"......11111111111111.................."<<std::endl;
//			vector<int> candidate;
//			candidate.clear();
//			for(int j=point_count_ogm_.ogmwidth_cell/2+4/point_count_ogm_.ogmresolution;j<point_count_ogm_.ogmwidth_cell/2+20/point_count_ogm_.ogmresolution;j++)//车体右侧
//			{
//
//				int count=0;
//				int index=i*point_count_ogm_.ogmwidth_cell+j;
//															//用于记录连续没有点云的栅格数
//								while(point_count_ogm_.ogm[index]<6&&count<maxz_ogm_.ogmwidth_cell/2+20/point_count_ogm_.ogmresolution-j)//0416zx==0变了
//								{
////									std::cout<<"......11111111111111.................."<<std::endl;
//									count++;
//									index++;
...
//								{
//									if(pathClear(i,j+count/2)||pathClear(i,j+count)	)	//判断一下由雷达到无点区域中点之间有无障碍 || pathClear(i,j)
//									{
////										std::cout<<"<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>"<<std::endl;
...
//								j+=count-1;
//			}
////			cout<<"the size of candidate is "<<candidate.size()<<endl;
////			for(int k=0;k<candidate.size()/3;k++){
////				int count=1;
////				while((candidate[3*k]-candidate[3*(k+1)])<2){
////					count++;
////					k++;
////				}
////				if(count>1){
...
////						vecright_.push_back(candidate[3*(k-counti)]);
////						vecright_.push_back(candidate[3*(k-counti)+1]);
////						vecright_.push_back(candidate[3*(k-counti)+2]);
////					}
////				}
////			}
//			candidate.clear();
...
//				int index=i*point_count_ogm_.ogmwidth_cell+j;
//															//用于记录连续没有点云的栅格数
//								while(point_count_ogm_.ogm[index]<2&&count<(maxz_ogm_.ogmwidth_cell/2-4/point_count_ogm_.ogmresolution-j))//0221这里的10之前是16zx
//								{
...
////										state|=pathClear(i,j+k);
////									}
//									if(pathClear(i,j)||pathClear(i,j+count/2))	//判断一下由雷达到无点区域中点之间有无障碍  ||pathClear(i,j+count)
//									{
//										memset(&ogm_msg_.ogm[index],5,count*4*sizeof(unsigned char));
//										memset(&ogm_msg_.ogm[index2],5,count*4*sizeof(unsigned char));
//										memset(&ogm_msg_.ogm[index3],5,count*4*sizeof(unsigned char));
...
//		以下为沿列方向搜索，
		for(int j=point_count_ogm_.ogmwidth_cell/2+4/point_count_ogm_.ogmresolution;j<point_count_ogm_.ogmwidth_cell/2+20/point_count_ogm_.ogmresolution;j++)//车体右侧
		{
			...
			for(int i=20/point_count_ogm_.ogmresolution;i<50/point_count_ogm_.ogmresolution;i++)
						{
...
//								continue;
							int count=0;
							int index=i*point_count_ogm_.ogmwidth_cell+j;
							while(point_count_ogm_.ogm[index]==0&&count<50/point_count_ogm_.ogmresolution-i)//&&count<point_count_ogm_.ogmheight_cell-i
							{
								count++;
...
//							int index_img=(point_count_ogm_.ogmheight_cell-i-1)*point_count_ogm_.ogmwidth_cell+j;//这是在图像上的索引，用于测试
							int thresh;
							if(i+count<45) thresh=GRID_THRESH;
							else if(i+count<55) thresh=GRID_THRESH2;
...
//									if(pathClear(i+count,j)){
//										count_inside++;
//									}
//								}
//								if(count_inside>count*0.7){state=true;}
								//求两端maxz
								float maxz1=-10,maxz2=-10;
								for(int k=4*(i-1)+3;k>4*(i-1);k--){//小栅格高
									for(int l=4*j;l<4*j+4;l++){//小栅格宽
										int small_i=k*maxz_ogm_.ogmwidth_cell+l;
										if(maxz_ogm_.ogm[small_i]>maxz1){
											maxz1=maxz_ogm_.ogm[small_i];
											if(abs(maxz1)>0.01) break;
										}
									}
									if(abs(maxz1)>0.01) break;
								}
								for(int k=4*(i+count);k<4*(i+count+1);k++){
									for(int l=4*j;l<4*j+4;l++){//小栅格宽
...
//										cout<<"..."<<k<<"..."<<l<<endl;
										...
										if(maxz_ogm_.ogm[small_i]>maxz2){
											...
											if(abs(maxz2)>0.01) break;
										}
									}
									if(abs(maxz2)>0.01) break;
								}
								float thresh_flat=(maxz1!=0&&maxz2!=0)?FLAT_THRESH:(-1);
//								if(maxz1!=0&&maxz2!=0){
//									cout<<"i is "<<i<<" j is "<<j<<endl;
//
//								}
//								cout<<"thresh is   "<<thresh<<endl;
								if((pathClear(4*i,j*4)&&pathClear((i+count/2)*4,4*j))&&pathClear((i+count)*4+3,4*j)&&abs(maxz1-maxz2)>thresh_flat)	//&&abs(maxz1-maxz2)>FLAT_THRESH
								{
...
//										std::cout<<ogm_msg_.ogm[index]<<" "<<ogm_msg_.ogm[index+ogm_msg_.ogmwidth_cell]<<std::endl;
		...
		for(int j=point_count_ogm_.ogmwidth_cell/2-4/point_count_ogm_.ogmresolution-1;j>point_count_ogm_.ogmwidth_cell/2-20/point_count_ogm_.ogmresolution;j--)//车体左侧
		{
...
//								}
//								if(count_inside>count*0.7){state=true;}
//								cout<<"i is  "<<i<<"j is "<<j<<"countis"<<count<<endl;

//								test
//								if(i==40&&j==4){
////									cout<<maxz_ogm_.ogm[(40+count+1)*maxz_ogm_.ogmwidth_cell+4]<<"is the maxz!!!!!!!!!"<<endl;
//									float maxz1=-10,maxz2=-10;
////									for(int k=4*(i-1)+3;k>4*(i-1)-1;k--){//小栅格高
////										for(int l=4*j;l<4*j+4;l++){//小栅格宽
////											int small_i=k*maxz_ogm_.ogmwidth_cell+l;
////											if(maxz_ogm_.ogm[small_i]>maxz1){
////												maxz1=maxz_ogm_.ogm[small_i];
...
////										if(abs(maxz1)>0.01) break;
////									}
////									cout<<"maxz1 is "<<maxz1<<endl;
//									int testcount=0;
//									for(int k=4*(i+count);k<4*(i+count+1);k++){
//										for(int l=4*j;l<4*j+4;l++){
//											testcount++;
//											int small_i=k*maxz_ogm_.ogmwidth_cell+l;
//											cout<<maxz_ogm_.ogm[small_i]<<endl;
//											if(maxz_ogm_.ogm[small_i]>maxz2){
//												maxz2=maxz_ogm_.ogm[small_i];
////												if(abs(maxz1)>0.01) break;
//											}
...
//										}
//									}
//									cout<<"maxz2is "<<maxz2<<endl;
//									cout<<"testcount is "<<testcount<<endl;
//								}
//								test
...
//										cout<<"..."<<k<<"..."<<l<<endl;
								...
								float thresh_flat=(maxz1!=0||maxz2!=0)?FLAT_THRESH:(-1);
//								cout<<"maxz1 is "<<maxz1<<" maxz2 is "<<maxz2<<endl;
//								if(maxz1!=0&&maxz2!=0){
//									cout<<"maxz1 is "<<maxz1<<" maxz2 is "<<maxz2<<endl;
//
//
//								cout<<"thresh is   "<<thresh_flat<<endl;


								if((pathClear(4*i,j*4)&&pathClear((i+count/2)*4,4*j))&&pathClear((i+count)*4+3,4*j)&&abs(maxz1-maxz2)>thresh_flat)	//(pathClear(4*i,j*4)&&pathClear((i+count/2)*4,4*j))&&pathClear((i+count)*4+3,4*j)&&abs(maxz1-maxz2)>thresh_flat
								{
		...
		for(int j=point_count_ogm_.ogmwidth_cell/2-4/point_count_ogm_.ogmresolution;j<point_count_ogm_.ogmwidth_cell/2+4/point_count_ogm_.ogmresolution;j++){//车辆前方
			for(int i=20/point_count_ogm_.ogmresolution+3/point_count_ogm_.ogmresolution;i<50/point_count_ogm_.ogmresolution;i++)
			{
				int thresh;
				if(i<45) thresh=GRID_THRESH;
				else if(i<55) thresh=GRID_THRESH2;
				...
				if(j>point_count_ogm_.ogmwidth_cell/2-3/point_count_ogm_.ogmresolution&&j<point_count_ogm_.ogmwidth_cell/2+3/point_count_ogm_.ogmresolution
					...
					continue;
				int index=i*point_count_ogm_.ogmwidth_cell+j;
				while(point_count_ogm_.ogm[index]<6&&count<50/point_count_ogm_.ogmresolution-i)//&&count<point_count_ogm_.ogmheight_cell-i
						{
					count++;
					index+=point_count_ogm_.ogmwidth_cell;
						}
				...
				if(count>thresh)								//可调参数
				{
					float maxz1=maxz_ogm_.ogm[tempindex],maxz2=maxz_ogm_.ogm[tempindex-(count+1)*maxz_ogm_.ogmwidth_cell];
					float thresh=(maxz1!=0&&maxz2!=0)?FLAT_THRESH:(-1);
					if(pathClear(4*i,4*j)&&abs(maxz1-maxz2)>thresh)	//判断一下由雷达到无点区域中点之间有无障碍  pathClear(j,i+count/2)
					{
						for(int k=0;k<count;k++)
						{
							//										ogm_msg_.ogm[index]=1;
	...
	{
		int height0=21/maxz_ogm_.ogmresolution,width0=20/maxz_ogm_.ogmresolution;//起点坐标zx
...
//			std::cout<<"..."<<"this is case2"<<std::endl;
			...
			{
				if(maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0]>PATHTHRESH){
//					if(maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0]>TREE||maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0+1]>TREE||maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0-1]>TREE){
//						return true;
//					}
					return false;
								}
	...
	void PostProcess::showOGM(const char* windowname ,const OGMData<int>& ogmdata,vector<int> vecside,vector<int> vecupdown)
	{
		...
		for(int j=0;j<ogmdata.ogmheight_cell;j++)
		{
			...
			for(int i=0 ;i < ogmdata.ogmwidth_cell ; i++)
			{
				...
				if(val > 0)
				{
					pdata[i*3]=abs(val*10);//以val×10作为像素值
					pdata[3*i+1]=255;
					pdata[3*i+2]=0;
				}
...
//			int height0=ogmdata.ogmheight_cell-1-21/ogmdata.ogmresolution,width0=20/ogmdata.ogmresolution;
//			int height=ogmdata.ogmheight_cell-1-5/ogmdata.ogmresolution,width=30/ogmdata.ogmresolution;
//			cvLine(slopemat,cvPoint(width0,height0),cvPoint(width,height),cvScalar(255));
//			float k=(float)(height-height0)/(width-width0);
//			float d=-0.5;
//			while(height>height0)
//			{
//				unsigned char* pdata = (unsigned char*)(slopemat->imageData + height0* slopemat->widthStep);
//				pdata[width0]=255;
////				if(maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0]>PATHTHRESH||maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0+1]>PATHTHRESH||maxz_ogm_.ogm[height0*maxz_ogm_.ogmwidth_cell+width0-1]>PATHTHRESH){
////					return false;}
//				height0++;
...
//	    	std::cout<<".............................."<<std::endl;
	...
	void PostProcess::radiusDetection(pcl::PointCloud<PointType>::Ptr cloud)
	{
...
//					std::cout<<"radiusdetectionenterrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"<<std::endl;
	...
	void PostProcess::radiusPointpair(pcl::PointCloud<PointType>::Ptr cloud)
	{
		int col_count = cloud->size() / LASER_LAYER;
		float x_thresh=6;
		float y_thresh=6;
	...
	void PostProcess::circleradiusDetection(pcl::PointCloud<PointType>::Ptr cloud)
	{
...
//	    }
	...
	void PostProcess::process()
	{
		...
		while(!processthreadfinished_)
		{
			...
			if(cloudmsg == nullptr)
				continue;
			pcl::PointCloud<PointType>::Ptr tempcloud(new pcl::PointCloud<PointType>);//当前帧点云（雷达里程计坐标系）
			pcl::fromROSMsg(*cloudmsg, *tempcloud);//获取当前帧点云数据
			std::vector<pcl::PointCloud<PointType>::Ptr> outputclouds;
			std::vector<PointType> lidarpropertys;
...
//				            stiff_name, 255, 0, 0);
					  	  countogmpoints(tempcloud);//outputclouds[0]);
